...
TEST(talos_experimental_walking, stepping)
{
  ...
  send_step_commands(walk_commands);

  ROS_INFO_STREAM("Sleeping until steps are finished");
  ros::Duration(parameters.test_duration_sec_).sleep();
  ROS_INFO_STREAM("Finished sleeping, checking final position");
  ...
  for (std::size_t i = 0; i < 2; ++i)
  {
    ...
    EXPECT_TRUE(pal::test_utils::isEigenMatrixNear(ref_pose.rotation(), actual_pose.rotation(),
                                                   parameters.final_orientation_tolerance_));
    EXPECT_TRUE(pal::test_utils::isEigenMatrixNear(ref_pose.linear(), actual_pose.linear(),
                                                   parameters.final_orientation_tolerance_));
