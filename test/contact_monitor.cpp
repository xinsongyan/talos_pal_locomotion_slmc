...
TEST(talos_pal_locomotion, contact_monitor)
{
  ...
  if (!set_model_srv.waitForExistence(ros::Duration(5.0)))
    PAL_THROW_DEFAULT("Couldn't start service gazebo/set_model_state");
  ...
  if(!model.initializeFromParameterServer(RigidBodyDynamics::FloatingBaseType::XYZ_Quaternion, nh))
    PAL_THROW_DEFAULT("Couldn't initialize model from param server");
  ...
  while(ros::Time::now() - start_time < ros::Duration(20.0))
  {
    ...
    if(isControllerActiveInList(list_controllers.response.controller, biped_controller))
      break;
  }
  ...
  Eigen::Vector3d robot_actual_pos;
  robot_actual_pos.setZero();

  for (std::size_t i = 0; i < 2; ++i)
  {
    Eigen::Isometry3d ref_pose = step_it->pose_;
    robot_actual_pos += ref_pose.translation();
    --step_it;
  }
  ...
  model_state.request.model_state.model_name = model_name;
  model_state.request.model_state.twist.linear.x = 0.0;
  model_state.request.model_state.twist.linear.y = 0.0;
  model_state.request.model_state.twist.linear.z = 0.0;
  model_state.request.model_state.twist.angular.x = 0.0;
  model_state.request.model_state.twist.angular.y = 0.0;
  model_state.request.model_state.twist.angular.z = 0.0;
  model_state.request.model_state.pose.position.x = robot_actual_pos(0) / 2;
  model_state.request.model_state.pose.position.y = robot_actual_pos(1) / 2;
  model_state.request.model_state.pose.position.z = 1.3;
  model_state.request.model_state.pose.orientation.x = 0.0;
  model_state.request.model_state.pose.orientation.y = 0.0;
  model_state.request.model_state.pose.orientation.z = 0.0;
  model_state.request.model_state.pose.orientation.w = 1.0;

  if (!set_model_srv.call(model_state))
    PAL_THROW_DEFAULT("Error calling set model pose");
  ...
  ROS_INFO("Finished contact monitor test");
}
